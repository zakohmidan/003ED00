...
echo -e "         \e[0m☠ \e[01;32mYASSIN0VITCH \e[0m☠\n\n\e[02mEssa BOOOYA \nhoooooo255 após o .\e[0m\n"; sleep 1
...
case $dots in
  ...
  2) ips=$(for i in {0..255}; do echo $IP$i.; done)
     ...
     ;;
  3) for i in {1..255}; do echo $IP$i >> ips.txt; done
      for i in $(seq $(wc -l < ips.txt)); do
         ip=$(sed -n "$i"p ips.txt)
         try=$(curl -m 0.5 -s -o /dev/null -w "%{http_code}" $DOMINIO -H "Upgrade: websocket" -x $ip:$PORTA)
          eval "case $try in
                 $nums\ )
                      echo -e \"\e[01;33m$ip\e[0m | \e[01;37mIP OK - STATUS $try\e[0m\"
                      echo \"$ip|$try\" >> OK.txt && paplay /usr/share/sounds/freedesktop/stereo/suspend-error.oga ;;
                   *)
